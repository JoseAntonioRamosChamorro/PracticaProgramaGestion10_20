...
public class PanBaja extends Frame implements WindowListener, ActionListener
{
	private static final long serialVersionUID = 1L;
	Label lblPan = new Label("Pan a borrar:");
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		else if(objetoPulsado.equals(btnSi))
		{
			...
			int respuesta = borrar(con, Integer.parseInt(Pan[0]));
			// Mostramos resultado
			if(respuesta == 0)
			{
				System.out.println("Pan eliminado");
			}
			else
			{
	...
	public int borrar(Connection con, int idPan)
	{
		int respuesta = 0;
		String sql = "DELETE FROM Pan WHERE idPan = " + idPan;
		System.out.println(sql);
		try 
		{
			Log4JCoreJava objLog4JCoreJavaSample = new Log4JCoreJava();
			  objLog4JCoreJavaSample.callMeInAppWarn("Se Elimino un campo");
